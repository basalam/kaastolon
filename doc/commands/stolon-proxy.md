...
```
  ...
  -h, --help                                 help for stolon-proxy
      ...
      --slave-node-exposure                  expose one of the slave node instead of master
      --slave-node-load-balance              weather to load balance between slave nodes or not (if true some times you cannot keep the connection to a single slave node)
